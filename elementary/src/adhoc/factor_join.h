...
#ifndef FACTOR_JOIN_H
#define FACTOR_JOIN_H

#include <fg/modelaccessor.h>
#include <fg/variable.h>

class Join_Factor_State{
public:
  int ffid;
  int aux;
  double running_sum;
...
public:
  ...
  static void load_factor(char * state, long sizeaux, int aux){
    Join_Factor_State * pstate = reinterpret_cast<Join_Factor_State*>(state);
    pstate->aux = aux;
  ...
  static bool variable_generate(char * state, long i_factor, Variable * propose, Variable * original){
    ...
    propose->rvalue = (pstate->running_sum >= 1);
