...
#include "sampler/uniformmetropolishastings.h"
...
double process_variable_sample(char * wr_copy, char * rd_copy, void * ppara, double* p_double1000buffer){
  ...
  if(pvar->stype == 'S'){
    ...
    if(pvar->dtype == 'C' && (pvar->upper <= 999 && pvar->lower >= 0)){
      ...
      changed = UniformMetropolisHastings<DRIVER, PAGER>::sample(factors, pvar, pvar_ori, p_double1000buffer, ModelAccessor::is_log_system);
